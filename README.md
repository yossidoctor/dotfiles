...
Wired via `core.hooksPath` in `git/identity`. Five checks, of which the first runs in every rule repo and the other four are `~/dotfiles`-only.

`scripts/rule-audit-gate.sh` refuses a commit whose staged rule files — `CLAUDE.md`, a skill, an agent definition, an output style — have no audit receipt for their exact content; `scripts/rule-audit.sh` is the audit that issues one, in a fresh `claude -p` session. Each script's header is the SoT for how it does that. A layer repo calls the same gate from its own `pre-commit`.

The other four checks: This repo is public, so the first is a denylist over the staged tree: a commit that names a private layer or an employer term fails and prints the line; the pattern lives in the hook, and the hook file itself is the one path excluded from its own scan. The second is `gitleaks` over the staged diff, for secret-shaped strings the denylist cannot know; GitHub's push protection is on for the repo as the backstop after a push. The third is `scripts/check-doc-refs.sh`, deployed on PATH as `check-doc-refs [--strict] [<root>]`: a skill citing a `.sh`/`.md` or a `skills/…` path that does not exist, or an `install.conf.yaml` `link:` naming an absent source, fails the commit; `§ Section` citations warn unless `--strict`. It knows no project, so a layer repo wraps it with its own passes and runs it against its own root. The fourth is `claude/hooks/tests/run-tests.sh` over the whole case table: a hook and its cases ship as one unit, and a case naming a skill or fixture the same commit deleted fails only at run time otherwise.
