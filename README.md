...
Permission rules match the command text, which `/bin/rm`, `sh -c '…'` or `git -C . push` slip past, so `sandbox` sits under them: every Bash-tool command runs inside macOS Seatbelt, writing only to the cwd, the session temp dir, `additionalDirectories` and `sandbox.filesystem.allowWrite`, with `~/.ssh` unreadable and `GITHUB_TOKEN`/`GH_TOKEN` unset (`sandbox.credentials`; `~/.config/gh` stays readable because gh's tokens live in the keychain and every `gh` call, the credential helper's included, needs that directory). Network goes through the sandbox's filtering proxy: the hosts a Bash command may reach are `sandbox.network.allowedDomains` (the public developer hosts here; a layer adds its own, and the arrays merge) plus whatever hosts the auto-mode classifier grants a single command; the proxy terminates TLS, so `enableWeakerNetworkIsolation` is on, which is the documented way to let `gh` verify certificates behind it. A host outside both lists is refused with its name in the command's result. A command the sandbox blocks falls back to the regular permission prompt, labelled unsandboxed (`allowUnsandboxedCommands: true`), and a sandbox that cannot start is a hard failure rather than a silent unsandboxed run (`failIfUnavailable`). Hooks, the statusline, LaunchAgents and `claude -p` audits run outside it.
