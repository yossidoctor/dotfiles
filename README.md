...
| `UserPromptSubmit` | `caffeinate-claude.sh` — keeps macOS awake while Claude works. `snapshot-rule-files.sh` — copies every rule file dirty against `HEAD` in each rule repo (`hook_rule_roots`) to `/tmp/claude-rule-snapshot-<session>/<repo>/` as the turn's baseline. |
