...
#           flag or a directory-wide token. Also `git commit -a`, which stages
...
#             staging content
#           - a path in a variable or a glob the shell expands — unknowable
#             here, and the commit's own staged-diff read is the backstop
...
#   . or :/ or :(top)                  a directory, and :/ the tree from root
