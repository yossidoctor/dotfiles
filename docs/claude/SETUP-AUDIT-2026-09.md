...
- **A network-unrestricted sandbox.** The sandbox pre-allows no host; in auto mode each command carries the hosts it needs, and the proxy terminates TLS, which `gh` refuses. `sandbox.network.allowedDomains` (public hosts in the base layer, work hosts in the private one) plus `enableWeakerNetworkIsolation` is the working shape; a new host shows up as a refusal naming it.
